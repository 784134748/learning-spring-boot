...
public class StringFormat2 {
     ...
     * @param format      格式化字符串
     * @param param       对象或Map&lt;String, Object&gt;类型的集合
     * @param postHandler 数据后处理接口，为NULL则将占位符变量中的所有NULL值替换为空字符串
     * @return
     */
    public static String format(String format, Object param, PostHandleInterface postHandler) {
        ...
        // 提取所有变量名，并有序存储
        TreeSet<String> varNameSet = new TreeSet<>();
    ...
    private static Set<String> extractVarReplacements(String targetStr) {
        ...
        Set<String> varReplacementSet = new HashSet<>();
     ...
     *
     * @param targetVarReplacementSet 由变量占位符组成的集合
     * @return
     */
    private static Map<String, String> extractVariable(Set<String> targetVarReplacementSet) {
        Map<String, String> varNameToVarReplacementMap = new HashMap<>();
     ...
     *
     * @param varReplacement 变量占位符,如“{variable}”
     * @return
     * @throws IllegalArgumentException
     */
    private static String extractVariable(String varReplacement) throws IllegalArgumentException {
        String varName = null;

        if (varReplacement == null) {
            // variableString = null;
        } else if (varReplacement.length() <= 2) {
            varName = "";
        } else {
            varName = varReplacement.substring(1, varReplacement.length() - 1).trim();
     ...
     * @param variablePrefix    变量前缀，比如：对于"aaa.bbb.ccc"中的"ccc"来说，它的变量前缀就是"aaa.bbb"（不带"."）
     * @param noneExtractKeySet 还没有没取值的键名集合
     * @param postHandler       数据读取后的处理接口，可自定义
     * @param returnData        待返回的数据
     */
    private static void extractValues(final Object targetObject, final String variablePrefix,
                                      final TreeSet<String> noneExtractKeySet, PostHandleInterface postHandler, Map<String, Object> returnData) {
        if (targetObject == null || noneExtractKeySet == null || noneExtractKeySet.isEmpty()) {
            return;
        }
        ...
        if (isPreKeyFinalVar) {
            ...
            Object rawObject = extractValueByPropName(targetObject, firstKey);
            String postHandledData = postHandler.handle(targetObject, firstKey, preFullVariableName, rawObject);
            returnData.put(preFullVariableName, postHandledData);
        } else {
            int separatorIndex = firstKey.indexOf('.');
            preKeyLocatedVar = getLocatedVariableFromFull(firstKey, separatorIndex);
            preKeySuccessorVar = getSuccessorVariableFromFull(firstKey, separatorIndex);
        ...
        for (String thisKey : pendingExtractKeySet) {
            ...
            if (isThisKeyFinalVar) {
                ...
                Object rawObject = extractValueByPropName(targetObject, thisKey);
                String postHandledData = postHandler.handle(targetObject, thisKey, fullVariableName, rawObject);
                returnData.put(fullVariableName, postHandledData);
            } else {
                int separatorIndex = thisKey.indexOf('.');
                thisKeyLocatedVar = getLocatedVariableFromFull(thisKey, separatorIndex);
                thisKeySuccessorVar = getSuccessorVariableFromFull(thisKey, separatorIndex);
        ...
        for (Entry<String, TreeSet<String>> entry : locatedVarToSuccessorVarSetMap.entrySet()) {
            ...
            Object thisObject = extractValueByPropName(targetObject, propName);
            // 递归调用，千万注意
            if (thisObject != null) {
     ...
     * 获得变量的全命名
     *
     * @param variablePrefix       变量的前缀（即包含这个变量的父变量）
     ...
     *
     * @param fullVariable   "aaa.bbb.ccc"
     * @param separatorIndex 分隔符位置
     * @return
     */
    private static String getLocatedVariableFromFull(String fullVariable, int separatorIndex) {
        return fullVariable.substring(0, separatorIndex);
    }
     ...
     * @return
     */
    private static String getSuccessorVariableFromFull(String fullVariable, int separatorIndex) {
        return fullVariable.substring(separatorIndex + 1);
    }
    ...
    public enum PostHandleType {
        ...
        private PostHandleInterface postHandler;

        PostHandleType(PostHandleInterface postHandler) {
            this.postHandler = postHandler;
        }

        public PostHandleInterface getPostHandler() {
            return postHandler;
        }
    ...
    public interface PostHandleInterface {
         ...
         * 从目标对象取出值后，进行后处理操作
         *
         * @param containerObject  取值的容器，即包含该值的对象
         * @param name             该值对应的属性名称，如“ccc”
         * @param fullVariableName 该值在占位符中的变量名称，如“aaa.bbb.ccc”
         * @param value            实际取值
         * @return
         */
     ...
     * @author mawenjian
     */
    public static class NullAsNullPostHandler implements PostHandleInterface {
     ...
     * @author mawenjian
     */
    public static class NullAsEmptyPostHandler implements PostHandleInterface {
     ...
     * @author mawenjian
     */
    public static class NullAsStringPostHandler implements PostHandleInterface {
